...
const (
	workerPoolFinalizer = "storm.apache.org/stormworkerpool-finalizer"
)

// StormWorkerPoolReconciler reconciles a StormWorkerPool object using state machines
...
func (r *StormWorkerPoolReconciler) initializeStateMachine(workerPool *stormv1beta1.StormWorkerPool) *state.StateMachine {
	...
	// If we have a known state, set the current state
	if currentState != state.State(state.WorkerPoolStateUnknown) {
		// Force the state machine to the correct state
		// This is a workaround since the state machine doesn't have a SetState method
		// We'll create a new one with the correct initial state and copy transitions
		sm = state.NewStateMachine(currentState)

		// Now add all the worker pool transitions to this state machine
		// From Unknown
		sm.AddTransition(state.State(state.WorkerPoolStateUnknown), state.Event(state.EventWPCreate), state.State(state.WorkerPoolStateCreating))

		// From Pending
		sm.AddTransition(state.State(state.WorkerPoolStatePending), state.Event(state.EventWPCreate), state.State(state.WorkerPoolStateCreating))

		// From Creating
		sm.AddTransition(state.State(state.WorkerPoolStateCreating), state.Event(state.EventWPCreateComplete), state.State(state.WorkerPoolStateReady))
		sm.AddTransition(state.State(state.WorkerPoolStateCreating), state.Event(state.EventWPCreateFailed), state.State(state.WorkerPoolStateFailed))

		// From Ready
		sm.AddTransition(state.State(state.WorkerPoolStateReady), state.Event(state.EventScaleUp), state.State(state.WorkerPoolStateScaling))
		sm.AddTransition(state.State(state.WorkerPoolStateReady), state.Event(state.EventScaleDown), state.State(state.WorkerPoolStateScaling))
		sm.AddTransition(state.State(state.WorkerPoolStateReady), state.Event(state.EventWPUpdateConfig), state.State(state.WorkerPoolStateUpdating))
		sm.AddTransition(state.State(state.WorkerPoolStateReady), state.Event(state.EventWPDrain), state.State(state.WorkerPoolStateDraining))
		sm.AddTransition(state.State(state.WorkerPoolStateReady), state.Event(state.EventWPDelete), state.State(state.WorkerPoolStateDeleting))
		sm.AddTransition(state.State(state.WorkerPoolStateReady), state.Event(state.EventWPHealthCheckFailed), state.State(state.WorkerPoolStateFailed))

		// From Scaling
		sm.AddTransition(state.State(state.WorkerPoolStateScaling), state.Event(state.EventWPScaleComplete), state.State(state.WorkerPoolStateReady))
		sm.AddTransition(state.State(state.WorkerPoolStateScaling), state.Event(state.EventWPScaleFailed), state.State(state.WorkerPoolStateFailed))

		// From Updating
		sm.AddTransition(state.State(state.WorkerPoolStateUpdating), state.Event(state.EventWPUpdateComplete), state.State(state.WorkerPoolStateReady))
		sm.AddTransition(state.State(state.WorkerPoolStateUpdating), state.Event(state.EventWPUpdateFailed), state.State(state.WorkerPoolStateFailed))

		// From Draining
		sm.AddTransition(state.State(state.WorkerPoolStateDraining), state.Event(state.EventWPDrainComplete), state.State(state.WorkerPoolStateReady))
		sm.AddTransition(state.State(state.WorkerPoolStateDraining), state.Event(state.EventWPDelete), state.State(state.WorkerPoolStateDeleting))

		// From Deleting
		sm.AddTransition(state.State(state.WorkerPoolStateDeleting), state.Event(state.EventWPDeleteComplete), state.State(state.WorkerPoolStateDeleted))

		// From Failed
		sm.AddTransition(state.State(state.WorkerPoolStateFailed), state.Event(state.EventWPRecover), state.State(state.WorkerPoolStatePending))
		sm.AddTransition(state.State(state.WorkerPoolStateFailed), state.Event(state.EventWPDelete), state.State(state.WorkerPoolStateDeleting))
	}
...
func (r *StormWorkerPoolReconciler) determineNextEvent(ctx context.Context, workerPoolCtx *WorkerPoolContext) (state.WorkerPoolEvent, error) {
	log := log.FromContext(ctx)
	...
	case state.WorkerPoolStateReady:
		...
		if err := r.checkWorkerPoolHealth(ctx, workerPool); err != nil {
			return state.EventWPHealthCheckFailed, nil
		}
...
func (r *StormWorkerPoolReconciler) executeStateAction(ctx context.Context, workerPoolCtx *WorkerPoolContext) error {
	currentState := workerPoolCtx.StateMachine.CurrentState()
	workerPool := workerPoolCtx.WorkerPool
...
func (r *StormWorkerPoolReconciler) createWorkerPool(ctx context.Context, workerPoolCtx *WorkerPoolContext) error {
	log := log.FromContext(ctx)
	workerPool := workerPoolCtx.WorkerPool
	topology := workerPoolCtx.Topology
	cluster := workerPoolCtx.Cluster
...
func (r *StormWorkerPoolReconciler) scaleWorkerPool(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) error {
	log := log.FromContext(ctx)
	...
	if deployment.Spec.Replicas == nil || *deployment.Spec.Replicas != workerPool.Spec.Replicas {
...
func (r *StormWorkerPoolReconciler) updateWorkerPool(ctx context.Context, workerPoolCtx *WorkerPoolContext) error {
	log := log.FromContext(ctx)
...
func (r *StormWorkerPoolReconciler) terminateWorkerPool(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) error {
...
func (r *StormWorkerPoolReconciler) checkResourcesCreated(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) error {
...
func (r *StormWorkerPoolReconciler) needsScaling(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) bool {
	...
	return deployment.Spec.Replicas == nil || *deployment.Spec.Replicas != workerPool.Spec.Replicas
}

// needsUpdate checks if worker pool needs update
func (r *StormWorkerPoolReconciler) needsUpdate(ctx context.Context, workerPoolCtx *WorkerPoolContext) bool {
...
func (r *StormWorkerPoolReconciler) checkWorkerPoolHealth(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) error {
	deployment := &appsv1.Deployment{}
	if err := r.Get(ctx, types.NamespacedName{
		Name:      workerPool.Status.DeploymentName,
		Namespace: workerPool.Namespace,
...
func (r *StormWorkerPoolReconciler) checkScalingComplete(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) error {
	deployment := &appsv1.Deployment{}
	if err := r.Get(ctx, types.NamespacedName{
		Name:      workerPool.Status.DeploymentName,
		Namespace: workerPool.Namespace,
...
func (r *StormWorkerPoolReconciler) checkUpdateComplete(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) error {
	deployment := &appsv1.Deployment{}
	if err := r.Get(ctx, types.NamespacedName{
		Name:      workerPool.Status.DeploymentName,
		Namespace: workerPool.Namespace,
...
func (r *StormWorkerPoolReconciler) canRecover(workerPool *stormv1beta1.StormWorkerPool) bool {
	// For now, always allow recovery attempts
	return true
}

// updateWorkerPoolStatus updates the worker pool status based on state
func (r *StormWorkerPoolReconciler) updateWorkerPoolStatus(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool, currentState state.State, errorMsg string) error {
...
func (r *StormWorkerPoolReconciler) updateComponentStatus(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) {
...
func (r *StormWorkerPoolReconciler) getRequeueDuration(currentState state.State) time.Duration {
	switch state.WorkerPoolState(currentState) {
	case state.WorkerPoolStateReady:
...
func (r *StormWorkerPoolReconciler) updateStatusError(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool, err error) (ctrl.Result, error) {
...
func (r *StormWorkerPoolReconciler) handleDeletion(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) (ctrl.Result, error) {
	log := log.FromContext(ctx)
...
func (r *StormWorkerPoolReconciler) reconcileDeployment(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool, topology *stormv1beta1.StormTopology, cluster *stormv1beta1.StormCluster) (*appsv1.Deployment, error) {
	log := log.FromContext(ctx)
...
func (r *StormWorkerPoolReconciler) buildWorkerDeploymentSpec(workerPool *stormv1beta1.StormWorkerPool, topology *stormv1beta1.StormTopology, cluster *stormv1beta1.StormCluster) appsv1.DeploymentSpec {
...
func (r *StormWorkerPoolReconciler) buildWorkerContainer(workerPool *stormv1beta1.StormWorkerPool, topology *stormv1beta1.StormTopology, cluster *stormv1beta1.StormCluster) corev1.Container {
	// Determine image
...
func (r *StormWorkerPoolReconciler) buildWorkerPodSpec(workerPool *stormv1beta1.StormWorkerPool, cluster *stormv1beta1.StormCluster, container corev1.Container) *corev1.PodSpec {
...
func (r *StormWorkerPoolReconciler) reconcileHPA(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool, deployment *appsv1.Deployment) error {
	log := log.FromContext(ctx)
...
func (r *StormWorkerPoolReconciler) buildHPASpec(workerPool *stormv1beta1.StormWorkerPool, deployment *appsv1.Deployment) autoscalingv2.HorizontalPodAutoscalerSpec {
...
func (r *StormWorkerPoolReconciler) reconcileService(ctx context.Context, workerPool *stormv1beta1.StormWorkerPool) error {
	log := log.FromContext(ctx)
...
func (r *StormWorkerPoolReconciler) getImagePullSecrets(cluster *stormv1beta1.StormCluster) []corev1.LocalObjectReference {
...
func (r *StormWorkerPoolReconciler) findWorkerPoolsForCluster(ctx context.Context, obj client.Object) []ctrl.Request {
...
func (r *StormWorkerPoolReconciler) findWorkerPoolsForTopology(ctx context.Context, obj client.Object) []ctrl.Request {
